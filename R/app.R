...
## Define UI                                                                 ##
###############################################################################
## All the elements are defined in ui_elements.R and exported so are available
## to be called as functions.
ui <- bslib::page_sidebar(
    title = shiny::h1("Lottie - Long-tailed Tit Data Capture"),
    sidebar = lottie::sidebar_accordion(),
    ...
    bslib::navset_card_underline(
      bslib::nav_panel(
        "Observations",
        lottie::gps_card(),
        lottie::observation_metadata_card(),
        lottie::flock_card(),
        lottie::individual_card(),
        lottie::interaction_card(),
        lottie::submit_card()
      ),
      bslib::nav_panel(
        "Download",
        lottie::download_card()
      )
    )
)
...
server <- function(input, output, session) {
  ...
  shiny::observeEvent(input$submit_conditions, {
    ## ns-rse - Reshape the data to wide as weather column can have multiple
    ## values and these expand to long format data
    to_add <- data.frame(
        user = input$user,
  ...
  shiny::observeEvent(input$add_description, {
    ...
    description_to_add <- rbind(description_data(), to_add)
