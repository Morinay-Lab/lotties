...
server <- function(input, output, session) {
  ...
  shiny::observeEvent(input$submit_conditions, {
    ...
    ## data
    to_add <- data.frame(user = input$user,
      ...
      tidyr::pivot_wider(names_from = weather,
                         values_from = present,
                         values_fill = FALSE)
  ...
  list(user = input$user,
       ...
       weather = paste(conditions_df$description[grepl(paste(input$conditions_weather,
                                                             collapse = "|"),
                                                       conditions_df$code)],
                       collapse = ", "),
  ...
  output$observation_user <- shiny::renderText({
    obs_metadata()$user
  })
  output$observation_date <- shiny::renderText({
    obs_metadata()$date
  })
  output$observation_start_time <- shiny::renderText({
    obs_metadata()$start_time
  })
  output$observation_end_time <- shiny::renderText({
    obs_metadata()$end_time
  })
  output$observation_weather <- shiny::renderText({
    obs_metadata()$weather
  })
  output$observation_visibility <- shiny::renderText({
    obs_metadata()$visibility
  })
  ...
  colour_ring <- shiny::reactive({
    input$composition_colour_ring
  })
  ...
  shiny::observe({
    ...
    rings <- lottie::extract_rings(code = input$composition_colour_ring,
  ...
  left_top_certain <- shiny::reactive({
    input$composition_left_top_certain
  })
  left_bottom_certain <- shiny::reactive({
    input$composition_left_bottom_certain
  })
  right_top_certain <- shiny::reactive({
    input$composition_right_top_certain
  })
  right_bottom_certain <- shiny::reactive({
    input$composition_right_bottom_certain
  })
  shiny::observe({
    if (left_top_certain() == FALSE | left_bottom_certain() == FALSE |
          right_top_certain() == FALSE | right_bottom_certain() == FALSE) {
  ...
  shiny::observeEvent(input$add_description, {
    ...
    to_add <-data.frame(
      ...
      stringsAsFactors = FALSE) |>
      dplyr::mutate(present = TRUE) |>
      tidyr::pivot_wider(names_from = other_species,
                         values_from = present,
                         values_fill = FALSE)
